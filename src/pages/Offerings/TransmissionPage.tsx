...
const { transmission } = offeringContent;

function TransmissionPage() {
  return (
    <>
      <MetaTags
        title="Transmission Line & Substations - Power Transmission Solutions"
        description="End-to-end EPC services for transmission lines and substations. Reliable power delivery with HT/LT lines, substations, and switchyards."
        canonical={`${SITE_URL}/offering/transmission`}
      />

      <PageHeader
        title={transmission.title}
        subtitle={transmission.subtitle}
        backgroundClass={transmission.backgroundClass}
      />
            ...
            <div className="col-lg-6 pe-lg-0" style={{ minHeight: '400px' }}>
              <div className="position-relative w-100 h-100">
                <img
                  ...
                  src={transmission.sections[0].image}
                  style={{ objectFit: 'cover' }}
                  alt="Transmission Line & Substations"
                  loading="lazy"
                />
              </div>
            </div>
                ...
                <ScrollAnimation animation="fade-slide-up">
                  <div className="row g-4">
                    <ul className="advantages-list">
                      {transmission.sections[0].content.map((text, index) => (
                        <p key={index}>{text}</p>
                      ))}
                    </ul>
      ...
      {/* FAQ Section */}
      <FAQSection faqs={transmission.faqs} accordionId="transmissionFaqAccordion" />
    </>
  );
}
